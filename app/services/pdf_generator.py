    ...
    def _build_process_analysis(self, process_name: str, process_stats: Dict, 
        ...
        if process_results:
            ...
            for result in process_results:
                ...
                # Tronca testi lunghi
                category = result['category'][:40] + '...' if len(result['category']) > 40 else result['category']
                dimension = result['dimension'][:35] + '...' if len(result['dimension']) > 35 else result['dimension']
            ...
            detail_table = Table(detail_data, colWidths=[2.8*inch, 2.2*inch, 0.7*inch, 1.5*inch])
